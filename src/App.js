...
// ─────────────────────────────────────────────────────────────────────
//  CONSUMER URL  →  sadiq-beef-shop.vercel.app
//  YOUR ADMIN    →  sadiq-beef-shop.vercel.app?admin
// ─────────────────────────────────────────────────────────────────────

const DEFAULT_CONFIG = {
  shopName:        "Sadiq Beef Shop",
  owner:           "Md Sabiq",
  whatsapp:        "919553334735",
  upi:             "9553334735@upi",
  upiName:         "Md Sabiq",
  bannerTitle:     "Farm Fresh Beef,\nAt Your Doorstep",
  bannerSub:       "☪️ 100% Halal | Fresh Daily Cuts | Visakhapatnam",
  announcements: [
    "☪️ 100% Halal certified — slaughtered as per Islamic law",
    "🚚 Delivery available across Visakhapatnam city",
    "💬 After placing order our team will WhatsApp you the delivery charge",
    "🥩 Fresh cuts available daily — order before 8 PM",
    "📞 Bulk orders? WhatsApp us directly!",
  ],
  importantNotices: [
    "☪️ 100% Halal certified — slaughtered as per Islamic law",
    "🚚 Delivery charge: 1 km = ₹15 (our team will confirm via WhatsApp)",
    "💬 After order, Md Sabiq will WhatsApp you the final total amount",
    "📍 Shop: Mustafa Colony, Arilova, Visakhapatnam",
  ],
  adminPin: "1234",
};
...
// ─────────────────────────────────────────────────────────────────────
//  LOCALSTORAGE — works on Vercel, all phones, all browsers
// ─────────────────────────────────────────────────────────────────────
const K = { orders:"sbshop-orders", products:"sbshop-products", shopOpen:"sbshop-open", config:"sbshop-config" };
...
const sendWA = (order, cfg) => {
  const items = order.items.map(i =>
    `  • ${i.emoji} ${i.name} (${i.weight}) × ${i.qty} = ₹${i.price * i.qty}`
  ).join("\n");
  const msg =
...
📍 *Note:* Please calculate delivery charge (₹15/km from Arilova) and WhatsApp the customer the final total amount before delivery.`;
...
const MeatLogo = ({ size=64 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100">
    ...
    <text x="50" y="84" textAnchor="middle" fontFamily="Georgia,serif" fontSize="8" fontWeight="bold" fill="#e8913a">Sadiq Beef Shop</text>
  </svg>
);
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:1000,
      ...
      <div style={{ background:`linear-gradient(160deg,#1a0e08,#0e0804)`,borderRadius:"24px 24px 0 0",
        ...
        <div style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:C.amb,fontWeight:800,textAlign:"center",marginBottom:6 }}>
          Add to Home Screen!
        </div>
        <div style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:C.mut,textAlign:"center",lineHeight:1.7,marginBottom:20 }}>
          Get quick access to <strong style={{ color:C.txt }}>{cfg.shopName}</strong> from your phone home screen — like a real app!
        </div>
        ...
        {isIOS && (
          <div style={{ background:C.sur,border:`1px solid ${C.bor2}`,borderRadius:14,padding:16,marginBottom:16 }}>
            ...
            {["1️⃣  Tap Share button (□↑) at bottom of Safari","2️⃣  Tap \"Add to Home Screen\"","3️⃣  Tap \"Add\" ✅"].map((s,i)=>(
        ...
        {isAndroid && (
          <div style={{ background:C.sur,border:`1px solid ${C.bor2}`,borderRadius:14,padding:16,marginBottom:16 }}>
            ...
            {["1️⃣  Tap 3-dot menu (⋮) in Chrome","2️⃣  Tap \"Add to Home screen\"","3️⃣  Tap \"Add\" ✅"].map((s,i)=>(
              <div key={i} style={{ fontFamily:"Lato,sans-serif",fontSize:12,color:C.mut,marginBottom:6,lineHeight:1.5 }}>{s}</div>
            ))}
          </div>
        )}
        {!isIOS && !isAndroid && (
          <div style={{ background:C.sur,border:`1px solid ${C.bor2}`,borderRadius:14,padding:14,marginBottom:16 }}>
            <div style={{ fontFamily:"Lato,sans-serif",fontSize:12,color:C.mut,lineHeight:1.6 }}>
              📱 On your phone → tap browser menu → <strong style={{ color:C.txt }}>"Add to Home Screen"</strong>
            </div>
          </div>
        )}
        <button onClick={onDismiss} style={{ width:"100%",background:`linear-gradient(135deg,${C.rdark},${C.red})`,
          color:"#fff",border:"none",borderRadius:16,padding:16,fontFamily:"Lato,sans-serif",
          fontWeight:700,fontSize:16,cursor:"pointer",boxShadow:`0 4px 20px ${C.red}55`,marginBottom:10 }}>
          🥩 Open Shop Now
        </button>
...
const Tog = ({ on, onChange, size="md" }) => {
  ...
  return (
    <div onClick={()=>onChange(!on)} style={{ width:w,height:h,borderRadius:h,cursor:"pointer",flexShrink:0,
      ...
      <div style={{ position:"absolute",top:"50%",transform:`translateY(-50%) translateX(${on?w-d-4:2}px)`,
...
const Btn = ({ children, onClick, sx={}, dis=false, v="pri" }) => {
  const base={fontFamily:"Lato,sans-serif",fontWeight:700,border:"none",cursor:dis?"not-allowed":"pointer",
    opacity:dis?.5:1,borderRadius:12,transition:"all .2s",letterSpacing:.5,
  ...
  <div style={{ background:`linear-gradient(160deg,#120800,#1e0d00)`,
    ...
    <div style={{ padding:"13px 18px 11px",display:"flex",alignItems:"center",gap:10 }}>
      ...
      <div style={{ flex:1,minWidth:0 }}>
        ...
        <div style={{ fontFamily:"Lato,sans-serif",fontSize:10,color:C.fnt,letterSpacing:1.5,textTransform:"uppercase",marginTop:1 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:6,flexShrink:0 }}>
          ...
          {canAdmin&&(
            <button onClick={onAdmin} style={{ background:"#1a1060",border:"1px solid #3a30a0",
              borderRadius:10,padding:"7px 11px",cursor:"pointer",color:"#a0a0ff",fontSize:13 }}>⚙️</button>
          )}
        </div>
      )}
...
const Home = ({ cart, onAdd, onRem, onCO, products, shopOpen, cfg }) => {
  ...
  if(!shopOpen) return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:30,textAlign:"center" }} className="fu">
  ...
  return (
    <div style={{ paddingBottom:totN>0?90:24 }}>
      ...
      <div style={{ margin:"14px 16px 0",background:"linear-gradient(135deg,#0d1e0d,#142814)",
        border:`2px solid ${C.halal}66`,borderRadius:16,padding:"14px 16px",display:"flex",alignItems:"center",gap:14 }}>
        ...
        <div style={{ margin:"10px 16px 0",background:"#1a0e00",border:`1px solid ${C.gold}55`,borderRadius:14,padding:"14px 16px" }}>
          ...
          {cfg.importantNotices.map((n,i)=>(
            <div key={i} style={{ display:"flex",alignItems:"flex-start",gap:8,marginBottom:i<cfg.importantNotices.length-1?8:0 }}>
              <div style={{ width:5,height:5,borderRadius:"50%",background:C.gold,marginTop:6,flexShrink:0 }}/>
      ...
      <div style={{ display:"flex",gap:8,padding:"4px 16px 10px",overflowX:"auto",scrollbarWidth:"none" }}>
        {cats.map(c=>(
          <button key={c} onClick={()=>setCat(c)} style={{ padding:"7px 15px",borderRadius:20,fontSize:12,
            whiteSpace:"nowrap",cursor:"pointer",fontFamily:"Lato,sans-serif",fontWeight:700,border:"none",
            transition:"all .2s",background:cat===c?C.red:C.sur,color:cat===c?"#fff":C.mut,
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,padding:"0 16px" }}>
        {list.map((p,i)=>(
          <div key={p.id} className="ch fu" style={{ background:`linear-gradient(160deg,${C.sur},#100806)`,
      ...
      {totN>0&&(
        <div onClick={onCO} style={{ position:"fixed",bottom:14,left:"50%",transform:"translateX(-50%)",
          width:"calc(100% - 32px)",maxWidth:420,background:`linear-gradient(90deg,${C.rdark},${C.red})`,
          borderRadius:16,padding:"15px 20px",cursor:"pointer",display:"flex",justifyContent:"space-between",
          alignItems:"center",boxShadow:`0 4px 24px ${C.red}55`,zIndex:200 }}>
          <div>
            <div style={{ fontFamily:"Lato,sans-serif",fontWeight:700,color:"#fff",fontSize:14 }}>{totN} item{totN>1?"s":""} in cart</div>
            <div style={{ fontFamily:"Lato,sans-serif",fontSize:11,color:"rgba(255,255,255,.75)" }}>Tap to checkout</div>
          </div>
          <div style={{ fontFamily:"'Playfair Display',serif",fontSize:18,color:"#fff",fontWeight:700 }}>₹{tot} →</div>
        </div>
      )}
    </div>
  );
...
const Cart = ({ cart, onAdd, onRem, onPlaced, cfg }) => {
  ...
  const [addr,setAddr]=useState(""); const [slot,setSlot]=useState(TIME_SLOTS[0]);
  ...
  const place=async()=>{
    ...
    const o={
      ...
      slot,items:cart,itemsTotal,
      status:"Order Placed",statusIndex:0,
      placedAt:new Date().toLocaleString("en-IN"),
    };
    const existing = lsGet(K.orders, []);
    lsSet(K.orders, [o, ...existing]);
    setBusy(false); onPlaced(o); sendWA(o,cfg);
  };
      ...
      {cart.length===0
        ...
        :<>
          ...
          <Card sx={{ marginTop:4 }}>
            ...
            <div style={{ fontFamily:"Lato,sans-serif",fontSize:12,color:"#d4b060",marginTop:10,lineHeight:1.6,
              background:"#1a0e00",border:`1px solid ${C.gold}44`,borderRadius:10,padding:"10px 12px" }}>
              🚚 Delivery charge not included here.<br/>
              Our team will calculate based on your address and send you the <strong style={{ color:C.gold }}>final total on WhatsApp</strong>.
            </div>
          </Card>
          ...
          <Card sx={{ marginTop:4 }}>
            ...
            <Fld label="Full Delivery Address" val={addr} set={setAddr} ph="House no, street, area, landmark, city..." multi/>
            <div>
              <label style={{ display:"block",fontSize:11,color:C.mut,textTransform:"uppercase",letterSpacing:1.2,marginBottom:8,fontFamily:"Lato,sans-serif",fontWeight:700 }}>
                Preferred Delivery Time Slot
              </label>
              <div style={{ display:"flex",flexWrap:"wrap",gap:8 }}>
                {TIME_SLOTS.map(s=>(
                  <button key={s} onClick={()=>setSlot(s)} style={{ padding:"8px 12px",borderRadius:9,fontSize:11,cursor:"pointer",
                    fontFamily:"Lato,sans-serif",fontWeight:700,border:"none",
                    background:slot===s?C.red:"#1a0e06",color:slot===s?"#fff":C.mut }}>{s}</button>
                ))}
              </div>
            </div>
          </Card>
          ...
          {/* What happens next */}
          <div style={{ background:"#0a1e0a",border:"2px solid #25d36655",borderRadius:14,padding:16,marginTop:4 }}>
            <div style={{ fontFamily:"'Playfair Display',serif",fontSize:15,color:"#25d366",fontWeight:700,marginBottom:10 }}>
              💬 What happens after you order?
            </div>
            {["Your order is sent to Md Sabiq on WhatsApp",
              "Our team calculates delivery from Arilova to your address",
...
const Confirm = ({ order, onHome, cfg }) => (
  <div style={{ padding:"24px 20px 30px",textAlign:"center" }} className="fu">
    ...
    <div style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:C.mut,marginBottom:2 }}>ID: <strong style={{ color:C.txt }}>{order.id}</strong></div>
    <div style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:C.mut,marginBottom:24 }}>
      Md Sabiq will WhatsApp you shortly!
    </div>
    <div style={{ background:"#0a1e0a",border:"1px solid #25d36655",borderRadius:16,padding:16,marginBottom:14,textAlign:"left" }}>
      <div style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:"#25d366",fontWeight:700,marginBottom:6 }}>💬 {cfg.owner} has been notified!</div>
      <div style={{ fontFamily:"Lato,sans-serif",fontSize:12,color:"#60c060",lineHeight:1.6 }}>
        Your order was sent to {cfg.owner} on WhatsApp. He will calculate the delivery charge and send you the <strong>final total via WhatsApp</strong>.
      </div>
    </div>
    <Card sx={{ textAlign:"left",marginBottom:14 }}>
      ...
      {[
        ...
        {s:"Team calculates delivery charge",done:false},
        {s:"Md Sabiq WhatsApps you the final amount",done:false},
        ...
        <div key={i} style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12 }}>
          ...
          <span style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:item.done?C.txt:C.fnt,fontWeight:item.done?700:400 }}>{item.s}</span>
        </div>
      ))}
    </Card>
    <Btn v="wa" onClick={()=>sendWA(order,cfg)} sx={{ width:"100%",marginBottom:10,padding:13 }}>💬 Resend to WhatsApp</Btn>
...
const ALogin = ({ onLogin, cfg }) => {
  ...
  return (
    <div style={{ padding:"60px 24px",textAlign:"center" }} className="fu">
      ...
      <input type="password" maxLength={8} value={pin} onChange={e=>{setPin(e.target.value);setErr(false);}}
...
const ProdModal = ({ prod, onSave, onClose }) => {
  const isNew=!prod.id;
  const [f,setF]=useState({ name:prod.name||"",weight:prod.weight||"500g",price:prod.price||"",
    category:prod.category||"Mince",emoji:prod.emoji||"🥩",tag:prod.tag||"Fresh",
    freshness:prod.freshness||"Today's Cut",inStock:prod.inStock!==undefined?prod.inStock:true,image:prod.image||null });
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center" }}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ background:"#1a0e08",borderRadius:"20px 20px 0 0",padding:"20px 20px 30px",
        width:"100%",maxWidth:430,maxHeight:"92vh",overflowY:"auto",border:`1px solid ${C.bor2}`,borderBottom:"none" }} className="fu">
    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.93)",zIndex:600,overflowY:"auto",padding:"20px 16px 50px" }} className="fu">
      <div style={{ maxWidth:430,margin:"0 auto" }}>
        ...
        <Card>
          ...
          <Fld label="Shop Name" val={f.shopName} set={v=>set("shopName",v)} ph="Sadiq Beef Shop"/>
        ...
        <Card>
          <SecTitle icon="📢" title="Scrolling Announcements"/>
        ...
        <Card>
          <SecTitle icon="⚠️" title="Important Notices Box"/>
...
const ChangePinModal = ({ cfg, onSave, onClose }) => {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ background:"#1a0e08",borderRadius:20,padding:24,width:"100%",maxWidth:380,border:`1px solid ${C.bor2}` }} className="fu">
        {ok?(
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontFamily:"Lato,sans-serif",fontSize:13,color:C.mut,marginBottom:20 }}>Your new PIN is saved.</div>
...
const Admin = ({ onLogout, products, setProducts, shopOpen, setShopOpen, cfg, setCfg }) => {
  ...
  const reload=()=>{ setLoad(true); const o=lsGet(K.orders,[]); setOrders(o); setLoad(false); };
  useEffect(()=>{ reload(); },[]);
  ...
  const updStatus=(id,d)=>{
    ...
    setOrders(u); lsSet(K.orders,u);
  };
  const delOrder=id=>{ const u=orders.filter(o=>o.id!==id); setOrders(u); lsSet(K.orders,u); };
  const saveProd=p=>{ const u=products.some(x=>x.id===p.id)?products.map(x=>x.id===p.id?p:x):[...products,p]; setProducts(u); lsSet(K.products,u); setEditP(null); };
  const delProd=id=>{ const u=products.filter(p=>p.id!==id); setProducts(u); lsSet(K.products,u); };
  const togStock=id=>{ const u=products.map(p=>p.id===id?{...p,inStock:!p.inStock}:p); setProducts(u); lsSet(K.products,u); };
  const saveCfg=c=>{ setCfg(c); lsSet(K.config,c); setShowCms(false); setShowPin(false); };

  const rev=orders.reduce((a,b)=>a+(b.itemsTotal||0),0);
  ...
  return (
    <div style={{ padding:16,paddingBottom:30 }} className="fu">
      ...
      {/* Edit Consumer View */}
      <div onClick={()=>setShowCms(true)} style={{ background:"linear-gradient(135deg,#1a3060,#2a60c0)",border:"2px solid #4a70d0",
      ...
      {/* Change PIN */}
      <div onClick={()=>setShowPin(true)} style={{ background:"linear-gradient(135deg,#3a1060,#6a30c0)",border:"2px solid #7a40d0",
        borderRadius:18,padding:"18px 20px",marginBottom:12,cursor:"pointer",
        display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div>
      ...
      <div style={{ background:shopOpen?"#0d2010":"#2a0a0a",border:`2px solid ${shopOpen?"#2e6e2e":"#6e1e1e"}`,
        ...
        <div>
          ...
          <div style={{ fontFamily:"Lato,sans-serif",fontSize:11,color:C.mut,marginTop:3 }}>
            {shopOpen?"Customers can order":"Customers see Closed screen"}
          </div>
        </div>
        <Tog on={shopOpen} onChange={v=>{ setShopOpen(v); lsSet(K.shopOpen,v); }}/>
      </div>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:16,overflowX:"auto" }}>
        {[["orders","📋 Orders"],["products","🥩 Products"],["stats","📊 Stats"]].map(([k,lbl])=>(
          <button key={k} onClick={()=>setTab(k)} style={{ padding:"8px 16px",borderRadius:10,border:"none",cursor:"pointer",
            whiteSpace:"nowrap",fontFamily:"Lato,sans-serif",fontWeight:700,fontSize:12,
            background:tab===k?C.red:C.sur,color:tab===k?"#fff":C.mut }}>{lbl}</button>
        ))}
        <button onClick={reload} style={{ marginLeft:"auto",background:C.sur,border:`1px solid ${C.bor2}`,borderRadius:10,color:C.mut,padding:"8px 12px",cursor:"pointer",fontSize:13 }}>🔄</button>
      </div>
              ...
              <div style={{ background:"#0e0804",borderRadius:10,padding:10,marginBottom:10 }}>
                {o.items.map(it=>(
                  <div key={it.id} style={{ display:"flex",justifyContent:"space-between",fontFamily:"Lato,sans-serif",fontSize:12,color:C.mut,marginBottom:4 }}>
                    <span>{it.emoji} {it.name} × {it.qty}</span><span style={{ color:C.amb }}>₹{it.price*it.qty}</span>
                  </div>
                ))}
                <div style={{ display:"flex",justifyContent:"space-between",fontFamily:"Lato,sans-serif",fontSize:13,color:C.amb,fontWeight:700,borderTop:`1px solid ${C.bor}`,paddingTop:8,marginTop:4 }}>
                  <span>Items Total</span><span>₹{o.itemsTotal}</span>
                </div>
                <div style={{ fontFamily:"Lato,sans-serif",fontSize:11,color:C.gold,marginTop:6,background:"#1a1000",borderRadius:8,padding:"6px 10px" }}>
                  🚚 WhatsApp customer to confirm delivery charge + final amount
                </div>
              </div>
              <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                {o.statusIndex<STATUS_FLOW.length-1&&<Btn v="suc" onClick={()=>updStatus(o.id,1)} sx={{ fontSize:12 }}>✅ "{STATUS_FLOW[o.statusIndex+1]}"</Btn>}
                {o.statusIndex>0&&<Btn v="ghost" onClick={()=>updStatus(o.id,-1)} sx={{ fontSize:11,padding:"7px 12px" }}>← Undo</Btn>}
      ...
      {tab==="products"&&(
        <div>
          <Btn onClick={()=>setEditP({inStock:true,emoji:"🥩",category:"Mince"})} sx={{ width:"100%",marginBottom:16,padding:13 }}>➕ Add New Product</Btn>
          {products.map(p=>(
            <Card key={p.id}>
        ...
        <div>
            ...
            {STATUS_FLOW.map((s,i)=>{
              ...
              return (<div key={s} style={{ marginBottom:12 }}>
                ...
                <div style={{ height:8,background:C.bor,borderRadius:4,overflow:"hidden" }}>
                  <div style={{ height:"100%",width:`${pct}%`,background:STATUS_COLORS[i],borderRadius:4 }}/>
                </div>
              </div>);
            })}
          </Card>
          <Card>
            ...
            {(()=>{
              ...
              const mx=sorted[0][1];
              return sorted.map(([nm,q])=>(<div key={nm} style={{ marginBottom:10 }}>
...
export default function App() {
  ...
  const [prods,setProds] = useState(()=> lsGet(K.products, DEFAULT_CATALOGUE));
  const [open,setOpen]   = useState(()=> lsGet(K.shopOpen, true));
  const [cfg,setCfg]     = useState(()=>({ ...DEFAULT_CONFIG, ...lsGet(K.config, {}) }));
  ...
  if(!ready) return (
    <div style={{ background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}><MeatLogo size={80}/>
